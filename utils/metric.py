...
def valid(model, device, dataset, view, data_size, class_num, eval_h=False):
    ...
    if eval_h:
        ...
        for v in range(view):
            ...
            y_pred = kmeans.fit_predict(low_level_vectors[v]/np.linalg.norm(low_level_vectors[v], axis=1, keepdims=True))
            nmi, ari, acc, pur = evaluate(labels_vector, y_pred)
            print('ACC{} = {:.4f} NMI{} = {:.4f} ARI{} = {:.4f} PUR{} = {:.4f}'.format(v + 1, acc, v + 1, nmi, v + 1, ari, v + 1, pur))
    ...
    return {'acc': acc, 'nmi': nmi, 'pur': pur}


def get_embedding(dataset, model, device, view, data_size):
    test_loader = DataLoader(
        dataset,
        batch_size=256,
        shuffle=False,
    ...
    for step, (xs, y, _) in enumerate(test_loader):
        for v in range(view):
            xs[v] = xs[v].to(device)
        with torch.no_grad():
            qs, _ = model.forward_cluster(xs)
            zs, _ = model.forward_plot(xs)
        for v in range(view):
            zs[v] = zs[v].detach()
            qs[v] = qs[v].detach()
            Zs[v].extend(zs[v].cpu().detach().numpy())
